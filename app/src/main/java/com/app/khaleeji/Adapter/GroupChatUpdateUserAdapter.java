...
public class GroupChatUpdateUserAdapter extends RecyclerView.Adapter<GroupChatUpdateUserAdapter.MyViewHolder> implements  Filterable {
    ...
    public GroupChatUpdateUserAdapter(Context context, boolean isAdmin, OnItemClickListener onItemClickListener) {
        this.context = context;
        this.onItemClickListener = onItemClickListener;
        this.isAdmin = isAdmin;
        if(SavePref.getInstance(context).getUserdetail() != null)
            myUsername = SavePref.getInstance(context).getUserdetail().getUsername();
    }

    @Override
    public MyViewHolder onCreateViewHolder( ViewGroup parent, int viewType) {

        View layoutView = LayoutInflater.from(parent.getContext()).inflate(R.layout.row_group_update_member, parent, false);
    ...
    public void onBindViewHolder(MyViewHolder holder, final int position) {
        ...
        holder.txtName.setText(friendlist.get(position).getUsername());

        if(isAdmin || myUsername.equals(friendlist.get(position).getUsername()))
